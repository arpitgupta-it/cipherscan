 ...
 * @param logoPath - The absolute path to the logo image to be included in the header of the report.
 ...
 */
export function generateHtmlReport(fileSummaries: string, fileDetails: string, logoPath: string, totalFilesScanned: number, secretsLength: number, context: ExtensionContext): string {
    // Construct the complete HTML structure for the report
    return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <link rel="stylesheet" href="file://${context.asAbsolutePath('src/assets/style/styles.css')}"> <!-- Link to external styles -->
    </head>
    <body>
        <header>
            <img src="file://${logoPath}" alt="CipherScan Logo"> <!-- Logo for the report -->
            <h1>CipherScan Secrets Report</h1> <!-- Main heading for the report -->
        </header>
        <main>
            ...
            <div class="boxed-section">
                ...
                <div class="note">
                    <strong>Note:</strong> 
                    For security reasons, exposed secrets are not listed in report; instead, users can identify them via metadata like 
