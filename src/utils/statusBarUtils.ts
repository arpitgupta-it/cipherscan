 ...
 * This function creates and configures a status bar item that allows the user to trigger a secret scan.
 * The item is displayed on the right side of the VS Code window with a command to start scanning for exposed secrets.
 * 
 ...
 * This function ensures that the status bar item is removed and cleaned up when the extension is deactivated.
 */
export function disposeStatusBar() {
    // Dispose of the status bar item to free up resources
    if (statusBarItem) {
