...
function promptForSecretScan(context: vscode.ExtensionContext): void {
    vscode.window
        .showInformationMessage(
            'You have committed changes. Run a scan for secrets before pushing?',
            'Yes',
            'No'
        )
        .then((selection) => {
...
export function watchGitCommit(context: vscode.ExtensionContext): void {
    ...
    try {
        ...
        gitWatcher.onDidChange(() => promptForSecretScan(context));
...
export async function addToGitIgnore(): Promise<void> {
    ...
    try {
        if (!fs.statSync(gitFolderPath).isDirectory()) return; // Ensure `.git` exists
        ...
        // Only show the information message if '.cipherscan' is not in .gitignore
